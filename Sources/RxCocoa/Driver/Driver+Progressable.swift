...
public extension SharedSequenceConvertibleType where SharingStrategy == DriverSharingStrategy {
    ...
    func handleShowLoading(with progressable: Progressable) -> Driver<E> {
        return self
            .do(onSubscribe: { [unowned progressable] in progressable.showLoading() })
    }
    
    /// Hides loading on first next event or completed event
    func handleHideLoading(with progressable: Progressable) -> Driver<E> {
        return self
    ...
    func handleLoading(with progressable: Progressable) -> Driver<E> {
        return self
